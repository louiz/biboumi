...
struct IrcServerOptions
{
  ...
  {
    this->set_tls_ports(old.tls_ports);
    this->set_ports(old.ports);
  }

  void set_ports(const std::string& str)
  {
    auto ports = utils::split(str, ':', false);
  ...
  void set_tls_ports(const std::string& str)
  {
    auto ports = utils::split(str, ':', false);
    std::transform(std::begin(ports), std::end(ports),
...
#pragma db id auto
  ...
  std::vector<uint16_t> tls_ports{6697, 6670};
  std::vector<uint16_t> ports{6667};
  std::string username;
  std::string realname;
  bool verify_cert{true};
  std::string trusted_fingerprint;
  std::string encoding_out{"ISO-8859-1"};
  std::string encoding_in{"ISO-8859-1"};
  int max_history_length{20};
};
