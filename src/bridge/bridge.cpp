    ...
    const std::string &channel_jid)
{
  ...
  p->add_to_inner(snippet_you_were_mentioned);

  {
    std::unique_ptr<XmlNode> a = std::make_unique<XmlNode>("a");
    a->add_to_inner(nick);
    (*a)["href"] = "xmpp:" + occupant_jid;
    a->add_to_tail(snippet_in);
          ...
          if (body.find(our_nick) != std::string::npos)
            {
              ...
              std::string channel_jid, occupant_jid;
              if (Config::get("fixed_irc_server", "").empty())
                {
                  ...
                  occupant_jid = std::to_string(author_iid) + "@" + this->xmpp.get_served_hostname();
                }
              else
                {
                  /* fixed server mode -> do not append the IRC network */
                  channel_jid = iid.get_local() + "@" + this->xmpp.get_served_hostname();
                  occupant_jid = nick + "@" + this->xmpp.get_served_hostname();
                }
